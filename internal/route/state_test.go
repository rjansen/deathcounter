...
func TestProcessTick_NotStarted(t *testing.T) {
	rs := NewRunState(testRoute())
	events := rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true}, IGT: 10000, DeathCount: 5})
	if len(events) != 0 {
...
func TestProcessTick_SingleCheckpoint(t *testing.T) {
	...
	events := rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true}, IGT: 95000, DeathCount: 3})
...
func TestProcessTick_FullProgression(t *testing.T) {
	...
	events = rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true, 2000: true}, IGT: 225000, DeathCount: 7})
	if len(events) != 1 {
	...
	events = rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true, 2000: true, 3000: true}, IGT: 400000, DeathCount: 10})
	if len(events) != 1 {
...
func TestProcessTick_NoNewFlags(t *testing.T) {
	...
	events := rs.ProcessTick(TickInput{Flags: map[uint32]bool{}, IGT: 10000, DeathCount: 0})
	if len(events) != 0 {
...
func TestProcessTick_AfterAbandon(t *testing.T) {
	...
	events := rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true}, IGT: 10000, DeathCount: 0})
	if len(events) != 0 {
...
func TestCurrentCheckpoint(t *testing.T) {
	...
	// Complete boss1
	rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true}, IGT: 95000, DeathCount: 0})
	cp = rs.CurrentCheckpoint()
	if cp == nil {
		t.Fatal("expected non-nil checkpoint")
	}
...
func TestCurrentCheckpoint_AllDone(t *testing.T) {
	...
	rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true, 2000: true, 3000: true}, IGT: 400000, DeathCount: 0})
	cp := rs.CurrentCheckpoint()
	if cp != nil {
...
func TestOptionalCheckpoints(t *testing.T) {
	...
	rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true, 2000: true}, IGT: 225000, DeathCount: 0})
	if !rs.IsComplete() {
...
func TestMultipleCheckpointsSameTick(t *testing.T) {
	...
	events := rs.ProcessTick(TickInput{Flags: map[uint32]bool{1000: true, 2000: true}, IGT: 200000, DeathCount: 5})
	if len(events) != 2 {
...
func TestMemCheck_LevelUp(t *testing.T) {
	...
	events := rs.ProcessTick(TickInput{
		MemValues: map[string]uint32{"level-20": 15},
		IGT: 60000, DeathCount: 0,
	})
	if len(events) != 0 {
		t.Fatalf("got %d events at level 15, want 0", len(events))
	}
	...
	if len(events) != 1 {
		t.Fatalf("got %d events at level 20, want 1", len(events))
	}
	if events[0].Checkpoint.ID != "level-20" {
		t.Errorf("got checkpoint %q, want level-20", events[0].Checkpoint.ID)
	}
}
...
func TestMemCheck_WeaponUpgrade(t *testing.T) {
	...
	if len(events) != 0 {
		t.Errorf("got %d events at +3, want 0", len(events))
	}
	...
	if len(events) != 1 {
		t.Fatalf("got %d events at +5, want 1", len(events))
	}
}
...
func TestMemCheck_GreaterThan(t *testing.T) {
	...
	if len(events) != 0 {
		t.Errorf("got %d events at 1000, want 0 (gt, not gte)", len(events))
	}
	...
	if len(events) != 1 {
		t.Fatalf("got %d events at 1001, want 1", len(events))
	}
}
...
func TestMixedFlagAndMemCheckpoints(t *testing.T) {
	...
	if len(events) != 1 {
		t.Fatalf("got %d events, want 1 (boss1 only)", len(events))
	}
	if events[0].Checkpoint.ID != "boss1" {
		t.Errorf("got %q, want boss1", events[0].Checkpoint.ID)
	}
	...
	if len(events) != 2 {
		t.Fatalf("got %d events, want 2 (level-30 + boss2)", len(events))
	}
	if events[0].Checkpoint.ID != "level-30" {
		t.Errorf("first: got %q, want level-30", events[0].Checkpoint.ID)
	}
	if events[1].Checkpoint.ID != "boss2" {
		t.Errorf("second: got %q, want boss2", events[1].Checkpoint.ID)
	}
	if rs.Status != RunCompleted {
